...
class RadixOrder extends H2O.H2OCountedCompleter<RadixOrder> {
  ...
  final int _sortDir;  // 0 to sort ASC, 1 to sort DESC

  RadixOrder(Frame DF, boolean isLeft, int whichCols[], int id_maps[][], int sortDir) {
    _DF = DF;
    _isLeft = isLeft;
  ...
  public void compute2() {
    ...
    if( _whichCols.length > 0 )
      new RadixCount(_isLeft, _base[0], _shift[0], _whichCols[0], _isLeft ? _id_maps : null, _sortDir).doAll(_DF.vec(_whichCols[0]));
    ...
    if( _whichCols.length > 0 )
      new SplitByMSBLocal(_isLeft, _base, _shift[0], keySize, batchSize, _bytesUsed, _whichCols, linkTwoMRTask, _id_maps, _sortDir).doAll(_DF.vecs(_whichCols)); // postLocal needs DKV.put()
  ...
  private void initBaseShift() {
    for (int i=0; i<_whichCols.length; i++) {
      ...
      if (col.isCategorical()) {
        ...
        if (col.isInt()) {
          _base[i] = BigInteger.valueOf(_sortDir==0?(long)colMin:(long)colMax*(-1));
          max = BigInteger.valueOf(_sortDir==0?(long)colMax:(long)colMin*(-1));
        } else{
          _base[i] = MathUtils.convertDouble2BigInteger(_sortDir==0?col.min():colMax*(-1));
          max = MathUtils.convertDouble2BigInteger(_sortDir==0?col.max():colMin*(-1));
        }
      }
