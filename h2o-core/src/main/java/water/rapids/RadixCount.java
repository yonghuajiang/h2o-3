...
class RadixCount extends MRTask<RadixCount> {
  ...
  RadixCount(boolean isLeft, BigInteger base, int shift, int col, int id_maps[][], int sortDir) {
    _isLeft = isLeft;
    _base = base;
    _col = col;
    _shift = shift;
      ...
      if (chk.vec().naCnt() == 0) { // no NAs in column
        ...
        for (int r = 0; r < chk._len; r++) {
          long ctrVal = isIntVal ?
                  BigInteger.valueOf(_sortDir==0?chk.at8(r):chk.at8(r)*(-1)).subtract(_base).add(BigInteger.ONE).shiftRight(_shift).longValue():
                  MathUtils.convertDouble2BigInteger(_sortDir==0?chk.atd(r):chk.atd(r)*(-1)).subtract(_base).add(BigInteger.ONE).shiftRight(_shift).longValue();
          tmp[(int) ctrVal]++;
        }
