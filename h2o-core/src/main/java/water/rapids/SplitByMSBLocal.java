...
class SplitByMSBLocal extends MRTask<SplitByMSBLocal> {
  ...
  SplitByMSBLocal(boolean isLeft, BigInteger base[], int shift, int keySize, int batchSize, int bytesUsed[], int[] col, Key linkTwoMRTask, int[][] id_maps, int sortDir) {
    _isLeft = isLeft;
     ...
     //long thisx = 0;
      ...
      if (!chk[0].isNA(r)) {
        ...
        if (_isLeft && _id_maps[0]!=null) { // dealing with enum columns
          ...
          thisx = isIntCols[0] ?
                  BigInteger.valueOf(_sortDir==0?chk[0].at8(r):chk[0].at8(r)*(-1)).subtract(_base[0]).add(BigInteger.ONE):
                  MathUtils.convertDouble2BigInteger(_sortDir==0?chk[0].atd(r):chk[0].atd(r)*(-1)).subtract(_base[0]).add(BigInteger.ONE);
          MSBvalue = thisx.shiftRight(_shift).intValue();
        }
      }
      ...
      for (int c=1; c<chk.length; c++) {  // TO DO: left align subsequent
        ...
        if (_isLeft && _id_maps[c] != null) thisx = BigInteger.valueOf(_id_maps[c][(int)chk[c].at8(r)] + 1);
          ...
          thisx =  isIntCols[c]?
                  BigInteger.valueOf(_sortDir==0?chk[c].at8(r):chk[c].at8(r)*(-1)).subtract(_base[c]).add(BigInteger.ONE):
                  MathUtils.convertDouble2BigInteger(_sortDir==0?chk[c].atd(r):chk[c].atd(r)*(-1)).subtract(_base[c]).add(BigInteger.ONE);
        }
