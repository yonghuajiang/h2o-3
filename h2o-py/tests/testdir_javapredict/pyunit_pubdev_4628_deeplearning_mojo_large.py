...
def deeplearning_mojo():
 ...
 #   params = set_params()   # set deeplearning model parameters
    params = {'missing_values_handling': 'Skip', 'use_all_factor_levels': False,
              'input_dropout_ratio': 0.4235599131613627, 'activation': 'tanh_with_dropout', 'standardize': True,
              'categorical_encoding': 'one_hot_internal', 'hidden': [19, 8, 15],
              'hidden_dropout_ratios': [0.43991155198334914, 0.22962287059602143, 0.07020708312932122],
              'autoencoder': False}
    ...
    try:
        ...
        print(str(type(ex)))
        if "EnvironmentError" in str(type(ex)):
            print("An unstable model is found and no mojo is built.")
        elif "AssertionError" in str(type(ex)):  # assertion error here.  Check mojo and pojo model here
        ...
        elif hasattr(ex, 'args') and type(ex.args[0])==type("what"):
...
#                 outFileName, "--mojo", os.path.join(TMPDIR, MOJONAME) + ".zip", "--decimal"]
